...
class Graphics:
    infile = r'D:\kernel\实习 · 简历\简历pdf\照片\无标题.jpg'
    outfile = r'D:\kernel\实习 · 简历\简历pdf\照片\transfer.jpg'
    ...
    def cut_by_ratio(cls, width, height):
        ...
        height = float(height)
        (x, y) = im.size
        region = (int((x-(x * (width / height)))/2), int((y-(y * (height / width)))/2),
                  int((x + (x * (width / height))) / 2), int((y+(y * (height / width)))/2))
