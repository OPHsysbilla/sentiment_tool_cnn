...
def loadStopWord(filename="stopwords1500.txt"):
    content = FIO.loadUTFBOMTxt(parentpath=cfg.stopWords_parent_dir,filename=filename)
    return content
...
""" 
    ...
    去除停用词    
"""
def delStopWord(string):
    noPunc = NoPunc(string)
    segList = jieba.cut(noPunc)
    # stopWords = loadStopWord()
    stopWords = GlobalStopWords
    stopWords.append(' ')
    new_list = [seg for seg in segList if not seg in stopWords]
    return new_list
...
def NoPunc(s):
    # 去除符号(所有符号)
    dropOtherSym = r'\W+'
    temp = re.sub(pattern=dropOtherSym,repl = ' ',string=s)
    # 去除连续空格
    blank_space = r'\s+'
    res = re.sub(pattern=blank_space,repl = ' ',string=temp)
    return res



if __name__ == "__main__":
    # pattern = r"(((\d+\.)?\d+[\u4e00-\u9fa5]?)|([+-])?\d+(\.\d+%?)?)|(\(\S+\))"
    pass